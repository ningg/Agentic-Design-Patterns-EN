...
> 提示链有时也称为流水线（Pipeline）模式，是在利用大语言模型（LLM）处理复杂任务时的一种有力范式。与其指望 LLM 在单一、整体的一步中解决复杂问题，提示链主张采用分而治之：把原本令人望而生畏的问题拆成一串更小、更易处理的子问题；每个子问题用专门设计的提示单独处理，且前一个提示的输出会有策略地作为链中下一个提示的输入。
...
> 这种顺序处理在与 LLM 的交互中天然带来模块性与清晰度。通过分解复杂任务，更容易理解与调试每一步，使整体流程更稳健、可解释。链中每一步都可以精心设计与优化，以聚焦更大问题中的某个具体方面，从而得到更准确、更聚焦的输出。
...
> 一步的输出作为下一步的输入至关重要。这种信息传递形成依赖链，因而得名「链」：先前操作的上下文与结果会引导后续处理，使 LLM 能在已有工作上继续构建、细化理解，并逐步逼近目标解。
...
> 此外，提示链不仅是拆解问题；它还能整合外部知识与工具。在每一步都可以指示 LLM 与外部系统、API 或数据库交互，从而超越其训练数据所固有能力。这大幅扩展了 LLM 的潜力，使其不仅能作为孤立模型，还能成为更广、更智能系统中的组成部分。
...
> 提示链的意义不止于解题；它还是构建复杂 AI 智能体（agent）的基础技术。这些智能体可利用提示链在动态环境中自主规划、推理与行动。通过有策略地排列提示顺序，智能体可承担需要多步推理、规划与决策的任务；此类工作流能更接近人类思维过程，从而与复杂领域和系统更自然、更有效地交互。
...
> **单条提示的局限：** 对多面任务而言，用一条复杂提示让 LLM 完成往往效率低下，模型会在约束与指令上吃力，并可能导致：指令忽视（提示的某些部分被忽略）、上下文漂移（偏离初始语境）、错误传播（早期错误被放大）、需要更长上下文窗口时信息不足以致难以回应，以及幻觉（认知负荷升高、错误信息概率上升）。例如，同一条查询要求分析市场调研报告、总结结论、用数据点识别趋势并起草邮件，很容易失败——模型可能总结得好，却抽不出数据或写不好邮件。
...
> **通过顺序分解提升可靠性：** 提示链把复杂任务拆成聚焦的、顺序的工作流，从而显著提升可靠性与可控性。就上文例子而言，流水线或链式做法可描述如下：
...
> 1. 初始提示（总结）：「请总结以下市场调研报告的关键发现：\[正文\]。」模型只专注总结，提高第一步准确度。  
> 2. 第二条提示（趋势识别）：「基于上述总结，识别三大新兴趋势，并提取支持每条趋势的具体数据点：\[第 1 步输出\]。」该提示更受约束，且直接建立在已验证的输出之上。  
...
> 这种分解便于对流程做更细粒度控制。每一步更简单、歧义更少，减轻模型认知负荷，使最终输出更准确、更可靠。该模块化类似计算流水线：每个函数完成特定操作再把结果交给下一步。为确保各子任务回答准确，可在每一阶段为模型指定不同角色。例如上述场景中，初始提示可标为「市场分析师」，其后为「行业分析师」，再后为「专业文档撰写人」，以此类推。
...
> **结构化输出的作用：** 提示链的可靠性高度依赖步骤间传递数据的完整性。若某步输出含糊或格式差，下一步可能因错误输入而失败。为缓解这一点，指定 JSON 或 XML 等结构化输出格式至关重要。
...
> 该结构化格式保证数据可被机器读取、精确解析并无歧义地填入下一步提示，从而减少解释自然语言时产生的错误，是构建稳健的多步 LLM 系统的关键组成部分。
...
> 在构建智能体系统时，提示链是适用面很广的模式；其核心效用在于把复杂问题拆成可顺序执行的、可管理的步骤。以下是若干实际应用与用例：
...
> 该方法用于自动内容分析、AI 研究助手开发、复杂报告生成等领域。
...
> 需要多步推理或信息检索才能回答的复杂问题，是典型用例。例如：「1929 年股市崩盘的主要原因是什么？政府政策如何应对？」
...
> * 提示 2：专门检索 1929 年崩盘原因相关信息。  
> * 提示 3：专门检索政府对 1929 年股市崩盘的政策应对信息。  
> * 提示 4：将第 2、3 步信息综合为对原始问题的连贯回答。
...
> 这种顺序处理方法是构建能进行多步推理与信息综合的 AI 系统的核心。当问题无法由单点数据回答，而需要一系列逻辑步骤或整合多源信息时，就需要这类系统。
...
> 例如，旨在就某主题生成全面报告的自动研究智能体会执行混合计算工作流：先检索大量相关文章；随后从每篇文章抽取关键信息的任务可对各来源并行执行，该阶段适合并行处理以最大化效率。
...
> 然而，各条抽取完成后，流程在本质上变为顺序的：须先汇总抽取结果，再综合成连贯草稿，最后审阅润色得到终稿；这些后续阶段在逻辑上依赖前一阶段成功完成。此处适用提示链：汇总数据作为综合提示的输入，综合后的文本再作为终稿审阅提示的输入。因此，复杂任务常将「独立数据收集」用并行处理，将「综合与润色等相互依赖步骤」用提示链结合。
...
> 将非结构化文本转为结构化格式通常需要迭代过程，通过顺序修改逐步提高输出的准确性与完整性。
...
> * 处理：检查必填字段是否齐全且格式是否符合要求。  
> * 提示 2（条件）：若字段缺失或格式错误，构造新提示让模型专门补全/修正，并可附上失败尝试的上下文。  
> * 处理：再次校验结果；必要时重复。  
> * 输出：给出已抽取且已校验的结构化数据。
...
> 该顺序处理方法特别适用于表单、发票、邮件等非结构化来源的抽取与分析。例如，处理 PDF 表单等复杂 OCR 问题，用分解的多步方法通常更有效。
...
> 首先用大语言模型从文档图像中做主要文字抽取；随后处理原始输出以规范化数据，例如把「one thousand and fifty」转为数值 1050。LLM 做精确算术是显著难点，因此在后续步骤中可把所需运算交给外部计算器：由 LLM 识别运算、把规范化后的数字交给工具，再纳入精确结果。文字抽取、数据规范化与外部工具使用这一链条，往往比单次 LLM 查询更可靠地得到准确终结果。
...
> 复杂内容的撰写通常是过程性任务，一般拆为若干阶段：初步构思、结构提纲、起草与后续修订
...
> * 提示 2：基于选定主题生成详细提纲。  
> * 提示 3：按提纲第一点写一节草稿。  
> * 提示 4：按提纲第二点写一节草稿，并附上上一节作为上下文；对提纲各点依此类推。  
...
> 尽管完整的状态管理架构往往比顺序链接更复杂，提示链仍为保持对话连续性提供基础机制：把每一轮对话构造成新提示，并系统性地纳入先前交互中的信息或抽取实体。
...
> * 后续轮次重复；每轮新用户话语启动一条利用累积对话历史（状态）的链。
...
> 该原则是对话智能体开发的基础，使其在长程多轮对话中保持上下文与连贯性；保存对话历史后，系统能理解并恰当回应依赖先前交流信息的用户输入。
...
> 生成可运行代码通常是多阶段过程，需要把问题分解为一串离散逻辑操作并逐步执行
...
> 在 AI 辅助软件开发等应用中，提示链的价值在于把复杂编码任务拆成一系列可管理的子问题，降低模型每步的操作复杂度。更重要的是，可在模型调用之间插入确定性逻辑，实现中间数据处理、输出校验与工作流内的条件分支，从而把原本可能导致不可靠或不完整结果的单次多面请求，转为由执行框架管理的有序操作序列。
...
> 分析包含多种模态的数据集需要把问题拆成更小的、基于提示的任务。例如，解读一张同时含嵌入文字、标注特定文字片段的标签，以及解释各标签的表格数据的图像，就需要这种做法。
...
> 实现提示链可从脚本内直接的顺序函数调用，到使用专门管理控制流、状态与组件集成的框架。LangChain、LangGraph、Crew AI 与 Google Agent Development Kit（ADK）等提供构建与执行这些多步流程的结构化环境，对复杂架构尤其有利。
...
> 演示可选用 LangChain 与 LangGraph：其核心 API 明确面向操作链与操作图的组合。LangChain 提供线性序列的基础抽象；LangGraph 扩展为有状态与循环计算能力，便于实现更复杂的智能体行为。本例聚焦基本的线性序列。
...
> 下列代码实现两步提示链，作为数据处理流水线：第一阶段解析非结构化文本并抽取信息；第二阶段接收抽取结果并转为结构化数据格式。
...
> 注意：`langchain-openai` 可替换为其他模型提供商的对应包。随后须为所选语言模型提供商（如 OpenAI、Google Gemini、Anthropic）配置执行环境所需的 API 凭证。
...
> 该 Python 示例展示如何用 LangChain 处理文本：两个独立提示分别用于从输入字符串抽取技术规格、以及将规格格式化为 JSON。语言交互使用 `ChatOpenAI`，`StrOutputParser` 保证输出为可用字符串。LangChain 表达式语言（LCEL）将提示与模型优雅地链接。`extraction_chain` 负责抽取；`full_chain` 将抽取结果作为转换提示的输入。示例提供描述笔记本电脑的样例输入，经两步处理后打印包含抽取与格式化规格的 JSON 字符串。
...
> 上下文工程（见图 1）是在生成 token 之前，为 AI 模型设计、构建并交付完整信息环境的系统学科。该方法认为，输出质量较少取决于模型架构本身，更多取决于所提供上下文的丰富程度。
...
> 图 1：上下文工程是为 AI 构建丰富、全面信息环境的学科；上下文质量是支撑高级智能体表现的主要因素之一。
...
> 相对主要优化用户即时查询措辞的传统提示工程，上下文工程是一次显著演进：范围扩展到多层信息，例如定义 AI 运行参数的**系统提示**（基础指令集）——如「你是技术写作者；语气须正式、精确。」上下文还通过外部数据增强：包括检索文档（AI 从知识库主动取数以支撑回答，如拉取项目技术规格），以及工具输出（AI 调用外部 API 的实时结果，如查日历判断用户是否有空）。显式数据与用户身份、交互历史、环境状态等关键隐式数据结合。核心原则是：即便先进模型在操作环境视图狭窄或构建不当时也会表现不佳。
...
> 因此，该实践把任务从「回答问题」重构为「为智能体构建完整操作图景」。例如，经上下文工程的智能体不会只答查询，而会先整合用户日历可用性（工具输出）、与邮件收件人的职业关系（隐式数据）、以往会议笔记（检索文档），从而生成高度相关、个性化且务实有用的输出。「工程」部分包括在运行时拉取与转换数据的稳健流水线，以及建立反馈回路以持续改进上下文质量。
...
> 实现上可使用专门调优系统大规模自动化改进。例如 Google Vertex AI 的提示优化器可对照样本输入与预定义评估指标系统评估回答以提升表现，便于在不同模型间适配提示与系统指令而无需大量手工改写。向优化器提供样本提示、系统指令与模板后，可程序化精炼上下文输入，为复杂上下文工程所需的反馈回路提供结构化手段。
...
> 这种结构化做法区分了简陋 AI 工具与更成熟、具上下文感知能力的系统：把上下文本身视为首要构件，强调智能体知道什么、何时知道、以及如何使用。它确保模型对用户意图、历史与当前环境有周全理解。归根结底，上下文工程是将无状态聊天机器人推进为高能力、情境感知系统的关键方法论。
...
> **是什么：** 复杂任务若在单条提示内处理，常使 LLM 不堪重负并引发明显性能问题；认知负荷升高会增加忽视指令、丢失上下文、生成错误信息等风险。整体式提示难以有效管理多重约束与顺序推理步骤，导致输出不可靠、不准确，无法覆盖多面请求的各个侧面。
...
> **为什么：** 提示链通过把复杂问题拆成更小、相互衔接的子任务序列，提供标准化解法。链中每一步用聚焦提示完成特定操作，显著提升可靠性与可控性；上一步输出作为下一步输入，形成逐步逼近最终解的逻辑工作流。模块化分治使流程更易管理、更易调试，并允许在步骤间整合外部工具或结构化数据格式。该模式是开发能规划、推理并执行复杂工作流的多步智能体系统的基础。
...
> **经验法则：** 当任务对单条提示过于复杂、包含多个明显处理阶段、需要在步骤间与外部工具交互，或构建需多步推理与状态维护的智能体系统时，采用本模式。
...
> 图 2：提示链模式：智能体接收用户的一系列提示，每个智能体的输出作为链中下一个的输入。
...
> 将复杂问题解构为更简单、可管理的子任务序列，提示链为引导大语言模型提供了稳健框架。「分而治之」通过每次只让模型专注一项具体操作，显著提升输出的可靠性与可控性。作为基础模式，它使开发具备多步推理、工具整合与状态管理能力的复杂 AI 智能体成为可能。归根结底，掌握提示链对构建能执行远超单条提示能力的复杂工作流、且具上下文感知能力的稳健系统至关重要。
