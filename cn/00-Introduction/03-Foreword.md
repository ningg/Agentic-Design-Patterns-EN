...
> 人工智能领域正处在一个引人入胜的拐点：我们正从仅能处理信息的模型，迈向能够推理、规划并行动以在模糊任务中达成复杂目标的智能系统。正如本书恰如其分地描述，这些「智能体化」系统代表了 AI 的下一前沿；其开发是一项令我们 Google 团队既兴奋又备受鼓舞的挑战。
...
> 《Agentic Design Patterns：构建智能系统的实践指南》恰逢其时，为我们指明这一旅程。本书正确地指出：作为这些智能体认知引擎的大语言模型之力，必须以结构与深思熟虑的设计加以驾驭。正如设计模式以共同语言与可复用方案改变了软件工程，本书中的智能体化模式也将成为构建稳健、可扩展且可信赖的智能系统的基础。
...
> 将构建智能体化系统比作「画布」的隐喻，与我们在 Google Vertex AI 平台上的工作深度共鸣。我们致力于为开发者提供最强、最灵活的画布，以构建下一代 AI 应用。本书提供实践性指导，帮助开发者充分发挥画布潜力；从提示链、工具使用到智能体间协作、自校正、安全与护栏，本书为希望构建复杂 AI 智能体的开发者提供了全面的工具箱。
...
> AI 的未来将由能够构建这些智能系统的开发者的创造力与巧思所定义。《Agentic Design Patterns》是释放这种创造力不可或缺的资源；它提供必要的知识与实例，不仅帮助理解智能体化系统的「是什么」与「为什么」，还有「如何做」。
...
> 我很高兴看到本书进入开发者社区。页中的模式与原则无疑将加速创新且影响深远的 AI 应用的发展，并在未来多年塑造我们的世界。
