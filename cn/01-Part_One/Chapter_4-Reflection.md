...
> 前几章覆盖了链式顺序执行、路由择路、并行化并发等基础模式，让智能体在处理复杂任务时更高效、更灵活。但即便流水线设计得很完善，第一版输出或计划仍可能欠准、不全或不够优——这时就要靠**反思**来兜底。
...
> 反思即智能体对自己的产出或过程做评估，再把评估结果用来改答案、调策略，属于自我纠错与自我改进：可依据外部反馈、内部批评，或与目标标准的差距反复迭代。也常见由「专职审稿」的另一智能体来完成这一步。
...
> 实务上很有效的一招，是把流程拆成两个逻辑角色：生产者与批评者（亦称生成器–批评者、生产者–审阅者）。单智能体也能自评，但拆成两个专用智能体（或同一模型下两套系统提示）通常更稳、更不容易「自己给自己放水」。
...
> 2. 批评者智能体：只干一件事——评生产者的稿；系统提示与人设与生产者不同（例如「资深工程师」「苛刻的事实核查员」）。对照事实、代码质量、文风、完整度等维度挑刺，输出结构化改进意见。
...
> 职责拆开以后，能缓解「既当球员又当裁判」的认知偏误：批评者带着找茬的使命进场。其反馈再喂回生产者，驱动第二版、第三版……本章 LangChain 示例用 `reflector_prompt` 固定批评者角色；ADK 示例则显式声明生产者与审阅者两个 `LlmAgent`。
...
> 反思相当于给系统加了一层「元认知」：不只产出答案，还审视产出过程，整体更聪明、更可靠。
...
> * 生产者–批评者拆分最常见：两个智能体或两套提示，换视角挑刺，反馈更结构化。  
